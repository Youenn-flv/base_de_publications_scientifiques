...
class Data :
    ...
    def filter_years(start_year, end_year):
        client = MongoAccess.connect()
        collection = client.publis 

        # publications = collection.find({'year': {'$gte': start_year, '$lte': end_year}})
    ...
    def filter_booktitles(booktitle):
        client = MongoAccess.connect()
        collection = client.publis 
        publications = collection.find({'booktitle': booktitle})
